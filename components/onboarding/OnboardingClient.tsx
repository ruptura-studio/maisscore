  ...
  useEffect(() => {
    fetch(`/api/onboarding/${token}`)
      ...
      .then((res) => {
        ...
        // Se onboarding já foi concluído anteriormente, redireciona direto
        if (d.onboardingCompletedAt) {
          setPageState("completed");
          setTimeout(() => {
            window.location.href = d.processSlug ? `/api/processo/${d.processSlug}` : "/meuprocesso";
          }, 0);
          return;
        }

        // Se todos os campos obrigatórios já vieram preenchidos, submete automaticamente
        const isCnpjLead = d.leadType === "cnpj";
        const requiredFilled =
          Boolean(d.birthDate) &&
          Boolean(d.addressZip) &&
          Boolean(d.addressStreet) &&
          Boolean(d.addressNumber) &&
          Boolean(d.addressNeighborhood) &&
          Boolean(d.addressCity) &&
          Boolean(d.addressState) &&
          Boolean(d.identityDocument) &&
          Boolean(d.civilStatus) &&
          Boolean(d.profession) &&
          Boolean(d.valorDivida) &&
          Boolean(d.objetivo) &&
          (!isCnpjLead || (Boolean(d.responsibleName) && Boolean(d.responsibleCpf)));
        ...
        if (requiredFilled) {
          ...
          fetch("/api/onboarding/complete", {
            ...
            body: JSON.stringify({
              ...
              addressZip: d.addressZip?.replace(/\D/g, ""),
              addressStreet: d.addressStreet,
              addressNumber: d.addressNumber,
              ...
              civilStatus: d.civilStatus,
              profession: d.profession,
              valorDivida: d.valorDivida || undefined,
              objetivo: d.objetivo,
              responsibleName: d.responsibleName || undefined,
              responsibleCpf: d.responsibleCpf?.replace(/\D/g, "") || undefined,
            }),
          })
            .then((r) => r.json())
            .then((res) => {
              const slug = res.processSlug ?? d.processSlug;
              setTimeout(() => {
