...
import { CheckCircle, AlertCircle, CheckCircle2, Circle, Check, TriangleAlert } from 'lucide-react'
...
function formatAcquisitionLabel(value: string | null | undefined, leadType: string | null | undefined) {
  ...
  if (leadType?.toLowerCase() === 'cpf') return 'Serviço jurídico de remoção de restrições em CPF.'
  if (leadType?.toLowerCase() === 'cnpj') return 'Serviço jurídico de remoção de restrições em CNPJ.'
...
function Input({
  ...
  pending,
  ...props
}: React.InputHTMLAttributes<HTMLInputElement> & { error?: string; prefilled?: boolean; answered?: boolean; pending?: boolean }) {
...
function TextareaState({
  ...
  error,
  ...props
}: React.TextareaHTMLAttributes<HTMLTextAreaElement> & { prefilled?: boolean; answered?: boolean; error?: boolean }) {
  const stateClass = prefilled
...
export default function OnboardingPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const [pageState, setPageState] = useState<'loading' | 'form' | 'completed' | 'invalid'>('loading')
  ...
  const [answeredFields, setAnsweredFields] = useState<Set<keyof FormState>>(new Set())
  ...
  function isAnsweredField(field: keyof FormState) {
    return prefilled.has(field) || answeredFields.has(field)
  }
  ...
  useEffect(() => {
    fetch(`/api/onboarding/${token}`)
      ...
      .then((res) => {
        ...
        setForm((prev) => ({
          ...prev,
          birthDate: d.birthDate ? new Date(d.birthDate).toLocaleDateString('pt-BR') : '',
          email: d.email ?? '',
          ...
          identityDocument: d.identityDocument ? formatIdentityDocument(d.identityDocument) : '',
  ...
  function validate(): boolean {
    ...
    if (form.phone.trim() && ![10, 11].includes(phoneDigits)) errs.phone = 'Telefone inválido'
    if (form.cpf.trim() && form.cpf.replace(/\D/g, '').length !== 11) errs.cpf = 'CPF inválido'
    if (!form.addressZip || form.addressZip.replace(/\D/g, '').length !== 8) errs.addressZip = 'CEP inválido'
    ...
    if (lead?.leadType === 'cnpj') {
      ...
      if (!form.responsibleCpf || form.responsibleCpf.replace(/\D/g, '').length !== 11) errs.responsibleCpf = 'CPF inválido'
    }
    setErrors(errs)
    ...
    try {
      await fetch('/api/onboarding/save-partial', {
        ...
        body: JSON.stringify({
          ...
          phone: normalizeBrazilPhone(form.phone) ?? (form.phone.replace(/\D/g, '') || undefined),
    ...
    try {
      const res = await fetch('/api/onboarding/complete', {
        ...
        body: JSON.stringify({
          ...
          email: form.email || undefined,
          phone: normalizeBrazilPhone(form.phone) ?? form.phone.replace(/\D/g, ''),
          cpf: form.cpf.replace(/\D/g, '') || undefined,
  ...
  if (pageState === 'invalid') {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 bg-neutral-100 px-4">
        ...
        <h1 className="font-dm text-h2 text-brand-navy text-center">Link inválido ou expirado</h1>
        <p className="text-sm text-foreground-alt text-center max-w-sm">
          Este link não é mais válido. Entre em contato com nossa equipe pelo WhatsApp para obter um novo link.
        </p>
        <a
  ...
  if (pageState === 'completed') {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 bg-neutral-100 px-4">
        ...
        <h1 className="font-dm text-h2 text-brand-navy text-center">Dados recebidos!</h1>
        <p className="text-sm text-foreground-alt text-center max-w-sm">
          Suas informações foram registradas com sucesso. Nossa equipe vai iniciar seu processo em breve e você receberá atualizações pelo WhatsApp.
        </p>
        <button
          type="button"
  ...
  const sectionComplete = {
    ...
    dados: hasVal(form.birthDate) && hasVal(form.identityDocument) && hasVal(form.civilStatus) && hasVal(form.profession),
    endereco: hasVal(form.addressZip) && hasVal(form.addressStreet) && hasVal(form.addressNumber) && hasVal(form.addressNeighborhood) && hasVal(form.addressCity) && hasVal(form.addressState),
    financeiro: hasVal(form.objetivo),
  }
    ...
    return (
      <AccordionTrigger className="flex w-full items-center justify-between py-5 hover:no-underline [&>svg]:text-brand-navy/40 [&[data-state=open]>svg]:rotate-180">
        <div className="flex items-center gap-3">
          <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${complete ? 'bg-[#4B9857]' : 'bg-brand-orange'}`}>
            {complete
              ? <Check className="h-3.5 w-3.5 text-white" strokeWidth={2.5} />
              : <TriangleAlert className="h-3.5 w-3.5 text-white" strokeWidth={2.5} />
            }
          </span>
          <span className={`text-xs font-semibold uppercase tracking-wide ${complete ? 'text-[#4B9857]' : 'text-brand-orange'}`}>{title}</span>
        </div>
      </AccordionTrigger>
    )
  }
  const acquisitionLabel = formatAcquisitionLabel(lead?.acquisition, lead?.leadType)
      ...
      <div className="mx-auto w-full max-w-[560px] px-4">
        ...
        <div className="mb-6">
          ...
          <p className="mt-6 text-sm leading-[18px] text-foreground-alt">
            É importante que todos os dados estejam preenchidos e corretos, pois usaremos estes dados para montar o processo jurídico e enviar para o tribunal.
          </p>
        </div>

        <div className="mb-6 rounded-lg bg-white p-5 shadow-sm">
          <h2 className="text-xs font-semibold uppercase tracking-wide text-brand-navy/60">Seu serviço contratado</h2>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div>
              <p className="text-xs text-foreground-alt">Serviço adquirido</p>
              <p className="mt-1 text-sm font-medium text-brand-navy">{acquisitionLabel}</p>
            </div>
            <div>
              <p className="text-xs text-foreground-alt">Data da aquisição</p>
              <p className="mt-1 text-sm font-medium text-brand-navy">{acquisitionDate}</p>
            </div>
          </div>
        </div>

        <p className="mb-6 text-sm leading-[18px] text-foreground-alt">
          Os campos em <span className="font-medium text-brand-orange">laranja</span> são informações que ainda precisamos.<br />Os <span className="font-medium text-green-600">verdes</span> já estão preenchidos, mas verifique antes de enviar.
        </p>

        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">

          <Accordion type="multiple" defaultValue={['contato', 'dados', 'endereco', 'financeiro']} className="flex flex-col gap-4">

          <AccordionItem value="contato" className="rounded-lg bg-white shadow-sm border-none">
            <div className="px-6">
              <SectionHeader title="Contato e processo" complete={sectionComplete.contato} />
            </div>
              ...
              {prefilled.has('phone') ? (
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('phone', formatPhone(e.currentTarget.value))}
                    error={errors.phone}
                    inputMode="tel"
              ...
              {prefilled.has('cpf') ? (
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('cpf', formatCpf(e.currentTarget.value))}
                    error={errors.cpf}
                    inputMode="numeric"
                    pending={!errors.cpf}
                  />
          ...
          {/* Dados pessoais */}
          <AccordionItem value="dados" className="rounded-lg bg-white shadow-sm border-none">
            <div className="px-6">
              <SectionHeader title="Dados pessoais" complete={sectionComplete.dados} />
            </div>
            <AccordionContent className="px-6 pb-6 pt-0 flex flex-col gap-4">

            <Field label="Data de nascimento" error={errors.birthDate} required>
              {prefilled.has('birthDate') ? (
                <PrefilledWrapper>
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onChange={(e) => handleFieldChange('birthDate', formatDate(e.target.value))}
                    onBlur={(e) => handleFieldBlur('birthDate', formatDate(e.currentTarget.value))}
            ...
            <Field label="RG ou CNH (número)" error={errors.identityDocument} required>
              {prefilled.has('identityDocument') ? (
                <PrefilledWrapper>
                  <Input type="text" value={form.identityDocument} prefilled />
                </PrefilledWrapper>
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onChange={(e) => handleFieldChange('identityDocument', formatIdentityDocument(e.target.value))}
                    onBlur={(e) => handleFieldBlur('identityDocument', formatIdentityDocument(e.currentTarget.value))}
            ...
            <Field label="Estado civil" error={errors.civilStatus} required>
              {prefilled.has('civilStatus') ? (
                <PrefilledWrapper>
                  <Select value={form.civilStatus} disabled>
                    <SelectTrigger className="h-11 border-success/40 bg-neutral-50 text-brand-navy/60 pl-9 cursor-default" data-radix-select-trigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <AnsweredWrapper>
                  <Select value={form.civilStatus} onValueChange={(v) => set('civilStatus', v)}>
                    <SelectTriggerState answered error={!!errors.civilStatus}>
                ...
                <PendingWrapper>
                  <Select value={form.civilStatus} onValueChange={(v) => set('civilStatus', v)}>
                    <SelectTrigger className={`h-11 pl-9 ${errors.civilStatus ? 'border-destructive' : 'border-dashed border-brand-orange/50'}`}>
                      <SelectValue placeholder="Selecione" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="solteiro">Solteiro(a)</SelectItem>
              ...
              {errors.civilStatus && <p className="text-txt-xs text-destructive">{errors.civilStatus}</p>}
            </Field>

            <Field label="Profissão" error={errors.profession} required>
              {prefilled.has('profession') ? (
                <PrefilledWrapper>
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('profession', e.currentTarget.value)}
          ...
          {/* Endereço */}
          <AccordionItem value="endereco" className="rounded-lg bg-white shadow-sm border-none">
            <div className="px-6">
              <SectionHeader title="Endereço" complete={sectionComplete.endereco} />
            </div>
            <AccordionContent className="px-6 pb-6 pt-0 flex flex-col gap-4">

            <Field label="CEP" error={errors.addressZip} required>
              {prefilled.has('addressZip') ? (
                <PrefilledWrapper>
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('addressZip', formatCep(e.currentTarget.value))}
            ...
            <Field label="Logradouro" error={errors.addressStreet} required>
              {prefilled.has('addressStreet') ? (
                ...
                <PendingWrapper>
                  <Input
                    type="text"
                    placeholder={zipLoading ? 'Buscando...' : 'Rua, Av., etc.'}
                    value={form.addressStreet}
                    onChange={(e) => handleFieldChange('addressStreet', e.target.value)}
                    onBlur={(e) => handleFieldBlur('addressStreet', e.currentTarget.value)}
              ...
              {prefilled.has('addressNumber') ? (
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('addressNumber', e.currentTarget.value)}
            ...
            <Field label="Bairro" error={errors.addressNeighborhood} required>
              {prefilled.has('addressNeighborhood') ? (
                <PrefilledWrapper>
                  <Input type="text" value={form.addressNeighborhood} prefilled />
                </PrefilledWrapper>
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('addressNeighborhood', e.currentTarget.value)}
            ...
            <Field label="Complemento">
              {prefilled.has('addressComplement') ? (
                <PrefilledWrapper>
                  <Input type="text" value={form.addressComplement} prefilled />
                </PrefilledWrapper>
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('addressComplement', e.currentTarget.value)}
                    pending
                  />
                </PendingWrapper>
              ...
              {prefilled.has('addressCity') ? (
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('addressCity', e.currentTarget.value)}
              ...
              {prefilled.has('addressState') ? (
                ...
                <PendingWrapper>
                  <Input
                    ...
                    onBlur={(e) => handleFieldBlur('addressState', e.currentTarget.value.toUpperCase().slice(0, 2))}
          ...
          {isCnpj && (
            <div className="rounded-lg bg-white shadow-sm">
              <div className="px-6 pt-5 pb-4 flex items-center gap-3">
                <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${hasVal(form.responsibleName) && hasVal(form.responsibleCpf) ? 'bg-[#4B9857]' : 'bg-brand-orange'}`}>
                  {hasVal(form.responsibleName) && hasVal(form.responsibleCpf)
                    ? <Check className="h-3.5 w-3.5 text-white" strokeWidth={2.5} />
                    : <TriangleAlert className="h-3.5 w-3.5 text-white" strokeWidth={2.5} />
                  }
                </span>
                <h2 className={`text-xs font-semibold uppercase tracking-wide ${hasVal(form.responsibleName) && hasVal(form.responsibleCpf) ? 'text-[#4B9857]' : 'text-brand-orange'}`}>Responsável legal</h2>
              </div>
              <div className="px-6 pb-6 flex flex-col gap-4">

              <Field label="Nome completo" error={errors.responsibleName} required>
                {prefilled.has('responsibleName') ? (
                  <PrefilledWrapper>
                    <Input type="text" value={form.responsibleName} prefilled />
                  </PrefilledWrapper>
                  ...
                  <PendingWrapper>
                    <Input
                      ...
                      onBlur={(e) => handleFieldBlur('responsibleName', e.currentTarget.value)}
              ...
              <Field label="CPF do responsável" error={errors.responsibleCpf} required>
                {prefilled.has('responsibleCpf') ? (
                  <PrefilledWrapper>
                    <Input type="text" value={form.responsibleCpf} prefilled />
                  </PrefilledWrapper>
                  ...
                  <PendingWrapper>
                    <Input
                      ...
                      onChange={(e) => handleFieldChange('responsibleCpf', formatCpf(e.target.value))}
                      onBlur={(e) => handleFieldBlur('responsibleCpf', formatCpf(e.currentTarget.value))}
          ...
          {/* Situação financeira */}
          <AccordionItem value="financeiro" className="rounded-lg bg-white shadow-sm border-none">
            <div className="px-6">
              <SectionHeader title="Situação financeira" complete={sectionComplete.financeiro} />
            </div>
            <AccordionContent className="px-6 pb-6 pt-0 flex flex-col gap-4">

              <Field label="Valor aproximado das dívidas">
              {prefilled.has('valorDivida') ? (
                <PrefilledWrapper>
                  <Select value={form.valorDivida} disabled>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="1000_5000">R$ 1.000 – R$ 5.000</SelectItem>
                      <SelectItem value="5000_20000">R$ 5.000 – R$ 20.000</SelectItem>
                      <SelectItem value="20000_50000">R$ 20.000 – R$ 50.000</SelectItem>
                      <SelectItem value="acima_50000">Acima de R$ 50.000</SelectItem>
                    </SelectContent>
                  </Select>
                </PrefilledWrapper>
              ) : hasValue(form.valorDivida) ? (
                <AnsweredWrapper>
                  <Select value={form.valorDivida} onValueChange={(v) => set('valorDivida', v)}>
                    <SelectTriggerState answered>
                ...
                <PendingWrapper>
                  <Select value={form.valorDivida} onValueChange={(v) => set('valorDivida', v)}>
                    <SelectTrigger className="h-11 pl-9 border-dashed border-brand-orange/50">
                      <SelectValue placeholder="Selecione (opcional)" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="ate_1000">Até R$ 1.000</SelectItem>
            ...
            <Field label="O que você quer alcançar com a regularização?" error={errors.objetivo} required>
              {prefilled.has('objetivo') ? (
                <PrefilledWrapper>
                  <TextareaState
                    disabled
                    value={form.objetivo}
                    rows={3}
                    className="w-full rounded-md border px-3 py-2.5 pl-9 text-sm outline-none resize-none cursor-default"
                  />
                </PrefilledWrapper>
              ) : isTypingField('objetivo') ? (
                <PendingWrapper>
                  <TextareaState
                    value={form.objetivo}
                    rows={3}
                    placeholder="Ex: Conseguir um financiamento, abrir empresa, alugar imóvel..."
                    onChange={(e) => handleFieldChange('objetivo', e.target.value)}
                    onBlur={(e) => handleFieldBlur('objetivo', e.currentTarget.value)}
                    error={!!errors.objetivo}
                    className="w-full rounded-md border px-3 py-2.5 pl-9 text-sm text-brand-navy placeholder:text-neutral-400 outline-none transition-colors resize-none"
                  />
                </PendingWrapper>
              ) : isAnsweredField('objetivo') ? (
                <AnsweredWrapper>
                  <TextareaState
                    value={form.objetivo}
                    rows={3}
              ...
              {errors.objetivo && <p className="text-txt-xs text-destructive">{errors.objetivo}</p>}
            </Field>
            </AccordionContent>
          </AccordionItem>
          ...
          <Dialog open={partialSaveAlert} onOpenChange={setPartialSaveAlert}>
            <DialogContent className="max-w-sm">
              <DialogHeader>
                ...
                <DialogTitle className="text-center font-dm text-brand-navy">Dados salvos parcialmente</DialogTitle>
                <DialogDescription className="text-center text-sm leading-[18px] text-foreground-alt">
                  Seus dados ainda não serão enviados, apenas salvamos os que já estão preenchidos para sua segurança.<br /><br />
                  Seu processo será iniciado apenas após o envio de todos os dados que precisamos.
                </DialogDescription>
              </DialogHeader>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="btn-secondary h-11 w-full rounded-md text-sm font-semibold disabled:opacity-50"
          >
            {submitting ? 'Enviando...' : 'Enviar dados'}
          </button>

          <p className="text-center text-txt-xs text-foreground-alt pb-4">
            Seus dados são protegidos pela LGPD e utilizados exclusivamente para o seu processo.
          </p>
        </form>
      </div>
    </div>
